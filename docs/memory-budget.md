...
an additional persistent offscreen render target. During a direct drag or the
90–220ms post-release settle only, one 1232×568 ARGB32 Cairo image
(2,799,104 bytes, 2.67 MiB) caches the fully rasterized drawer. This avoids
redrawing text and rounded paths on every frame; it is destroyed immediately
when the interaction settles. The image-level acceptance test records RSS/PSS
for the actual linked library set and rejects persistent growth.
